...
%token WHILE IF RETURN PREPROC LE STRING PRINT FUNCTION DO ARRAY ELSE STRUCT STRUCT_VAR FOR GE EQ NE BREAK CONTINUE INC DEC
%right '='
...
stmt : Declaration
	...
	| ';'
        | BREAK ';' {if(!loop) printf("\nError: Wrong usage of break: Line %d\n",printline());errc++;}
        | CONTINUE ';' {if(!loop) printf("\nError: Wrong usage of continue: Line %d\n",printline());errc++;}
	| PRINT '(' STRING ')' ';'
	| CompoundStmt
	;
